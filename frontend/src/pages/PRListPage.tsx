...
import { Plus, Search, Eye, FileText, AlertCircle, Calendar } from 'lucide-react';
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        ...
        {user?.role === 'REQUESTER' || user?.role === 'ADMIN' ? (
          <button className="inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-xl shadow-sm text-white bg-primary-600 hover:bg-primary-700 focus:outline-none transition-all hover:shadow-md hover:-translate-y-0.5"
            onClick={() => window.location.href='/pr/new'}>
        ...
        <div className="p-6 border-b border-slate-100 bg-slate-50/50">
          <div className="relative max-w-md">
            ...
            <input
              ...
              placeholder="Search by PR Number or Justification..."
              value={searchTerm}
              ...
              {loading ? (
                ...
                filteredPRs.map((pr) => (
                  <tr key={pr.prNumber} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm text-slate-900 max-w-xs truncate" title={pr.justification}>
                        {pr.justification}
                      </div>
                      <div className="text-xs text-slate-500 mt-1">
                        Required by: {formatDate(pr.requiredDate)}
                      </div>
                    </td>
